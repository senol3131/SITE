...
<div class="content-title">
	<h1 class="title"><center>Yıllık Sıralama</center></h1>
</div><!-- content-title -->
...
<?php
				$yearly = $dbo->doquery("SELECT TOP 100 UP.shIndex, UP.strElmoUserID, UP.nElmoLoyalty, UD.Class, KD.IDNUM, KD.IDName AS KnightName FROM USER_KNIGHTS_RANK AS  UP  LEFT JOIN USERDATA  AS UD ON UD.strUserID = UP.strElmoUserID LEFT JOIN KNIGHTS AS KD ON UD.Knights = KD.IDNUM WHERE UD.AUTHORITY = 1 ORDER BY UP.shIndex");
				$i = 0;
				while($dbo->row($yearly)):
				$index = $dbo->result('shIndex');
				$elmouser = $dbo->result('strElmoUserID');
				$clanname  = $dbo->result('KnightName');
				$elmonp = $dbo->result('nElmoLoyalty');
...
<?php
				$yearly = $dbo->doquery("SELECT TOP 100 UP.shIndex, UP.strKarusUserID, UP.nKarusLoyalty, UD.Class, KD.IDNUM, KD.IDName AS KnightName FROM USER_KNIGHTS_RANK AS  UP  LEFT JOIN USERDATA  AS UD ON UD.strUserID = UP.strKarusUserID LEFT JOIN KNIGHTS AS KD ON UD.Knights = KD.IDNUM WHERE UD.AUTHORITY = 1 ORDER BY UP.shIndex");
				$i = 0;
				while($dbo->row($yearly)):
				$index = $dbo->result('shIndex');
				$elmouser = $dbo->result('strKarusUserID');
				$clanname  = $dbo->result('KnightName');
				$elmonp = $dbo->result('nKarusLoyalty');
				$job 	 = $dbo->result('Class');
				$idnum 	 = $dbo->result('IDNUM');
				$i++;

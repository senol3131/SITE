...
<div class="content-title" style="float: left;width: 100%;">
	<h1 class="title"><center>Şifre Sıfırlama</center></h1>
</div><!-- content-title -->
...
<?PHP
 ...
 if(isset($_POST['submit'])):
	...
	$yenisifre  =   $dbo->SQLSecurity($_POST['newpass']);	
	...
	$MyKOLCevap = $dbo->result(2);
		...
		if(!$yenisifre):
			...
			$dbo->yonlendir(''.$base_url.'UserCP/ChangePW',0);			
		elseif(preg_match("/[^a-z, A-Z, 0-9]/", $yenisifre)):
			$dbo->uyari(''.$lang['gecerlisifre'].' '.$lang['gecerlikharf'].' : abcdefghijklmnoprstuvyzxwq '.$lang['gecerlibahrf'].' : ABCDEFGHIJKLMNOPRSTUVYZXWQ '.$lang['gecerlirakam'].': 0123456789');
			$dbo->yonlendir(''.$base_url.'UserCP/ChangePW',0);
		elseif($MyKOLSoru <> $gsoru or $MyKOLCevap <> $gyanit):
			$dbo->uyari(''.$lang['gsoruyanlis'].'');
			$dbo->yonlendir(''.$base_url.'UserCP/ChangePW',0);
		else:
			$dbo->doquery("UPDATE TB_USER Set StrPasswd = dbo.HashPasswordString('$yenisifre') where StrAccountID = '$username'");
 ...
 echo'
<form action="'.$base_url.'UserCP/ChangePW" method="post">   
		<center>

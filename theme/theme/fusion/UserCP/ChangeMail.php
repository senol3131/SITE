...
<div class="content-title" style="float: left;width: 100%;">
	<h1 class="title"><center>Mail Adresi Değiştirme</center></h1>
</div><!-- content-title -->
...
<?PHP
 ...
 if(isset($_POST['submit'])):
	...
	$MyKOLCevap = $dbo->result(2);
		...
		if(!$mail):
			...
			$dbo->yonlendir(''.$base_url.'UserCP/ChangeMail',0);
		...
		elseif($MyKOLSoru <> $gsoru or $MyKOLCevap <> $gyanit):
			...
			$dbo->doquery("UPDATE _Odestashield_mykol Set MyKOLMail = '$mail' where StrAccountID = '$username'");
 ...
 echo'
<form action="'.$base_url.'UserCP/ChangeMail" method="post">   
		<center>
...
<input type="text" required="true" style="display: block;
   color: white;
    ...
    border: 2px solid rgb(7, 20, 33);
    box-shadow: 0px 0px 12px 0px rgba(0, 0, 0, 0.62);
" name="mail" placeholder="Yeni Mailiniz">

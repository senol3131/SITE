...
<div class="content-title" style="float: left;width: 100%;">
	<h1 class="title"><center>OTP Şifresi Değiştirme</center></h1>
</div><!-- content-title -->
...
<?PHP
 ...
 if(isset($_POST['submit'])):
	...
	$otp  		=   $dbo->SQLSecurity($_POST['newpass']);
	...
	$MyKOLCevap = $dbo->result(2);
		...
		if(!$otp):
			...
			$dbo->yonlendir(''.$base_url.'UserCP/ChangeOTP',0);
		...
		elseif($MyKOLSoru <> $gsoru or $MyKOLCevap <> $gyanit):
			...
			$dbo->doquery("UPDATE TB_USER Set OTPPassword = '$otp' where StrAccountID = '$username'");
 ...
 echo'
<form action="'.$base_url.'UserCP/ChangeOTP" method="post">   
		<center>
...
<input type="text" maxlength="6" required="true" style="display: block;
   color: white;
    text-align: center;
    width: 64%;
    margin: 16px auto;
    ...
    border: 2px solid rgb(7, 20, 33);
    box-shadow: 0px 0px 12px 0px rgba(0, 0, 0, 0.62);
" name="newpass" onkeypress="return onlyNumbers();" placeholder="Yeni OTP Şifreniz">

		
  	<br>
  <input type="submit" name="submit" value="Mailimi Değiştir" style="cursor:pointer;background: #342d98;background: url('.$path.'images/login-button.jpg) no-repeat;background-size: cover;margin-top: 20px;padding: 10px 46px;,: 35pxline-height: 13px;font-size: 14px;font-weight: bold;box-shadow: 0px 0px 29px 1px #1b532d;">
		</center>
	</form>
 ';
